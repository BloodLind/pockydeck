...
data class SettingsScreenState(val confirmBackMapping: ConfirmBackMapping, val defaultHomeSummary: String, val categorySummaries: List<CategorySummary> = emptyList())
data class SettingsCallbacks(val onSetConfirmBackMapping: (ConfirmBackMapping) -> Unit, val onRequestDefaultHome: () -> Unit, val onOpenSystemAction: (String) -> Unit, val onOpenCategory: (LibraryCategory) -> Unit, val onFocusedAction: OnFocusedAction = {})

private val settingsSections = listOf("Controls", "Launcher", "Android")
...
fun SettingsScreen(
    ...
    if (maxWidth > 600.dp) Row(Modifier.fillMaxSize(), horizontalArrangement = Arrangement.spacedBy(LauncherTheme.spacing.lg)) {
        Column(Modifier.width(176.dp), verticalArrangement = Arrangement.spacedBy(LauncherTheme.spacing.xs)) {
...
private fun SettingsBody(state: SettingsScreenState, callbacks: SettingsCallbacks, systemActions: List<SupportedSystemAction>, section: String, modifier: Modifier) {
    ...
    LaunchedEffect(section, systemActions.size, hasInitialControl) {
    ...
    Column(modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(LauncherTheme.spacing.sm)) {
        ...
        if (section == "All" || section == "Controls") {
            ...
            ChoiceRow("Confirm button", state.confirmBackMapping.confirm.name,
                ...
                onFocusChanged = focus("Swap Confirm and Back", LauncherActionMeaning.CHANGE_FILTER, { callbacks.onSetConfirmBackMapping(swapped) }, callbacks.onFocusedAction))
        }
        if (section == "All" || section == "Launcher") {
            ...
            ActionRow("Set as Home launcher", state.defaultHomeSummary,
                ...
                onFocusChanged = focus("Set as Home launcher", LauncherActionMeaning.OPEN_SETTINGS, callbacks.onRequestDefaultHome, callbacks.onFocusedAction))
            LauncherText(
            ...
            state.categorySummaries.forEach { summary -> ActionRow(category(summary.category), "${summary.count} items", onActivate = { callbacks.onOpenCategory(summary.category) },
                onFocusChanged = focus(category(summary.category), LauncherActionMeaning.CHANGE_FILTER, { callbacks.onOpenCategory(summary.category) }, callbacks.onFocusedAction)) }
        }
        if (section == "All" || section == "Android") {
            ...
            systemActions.forEachIndexed { index, action -> ActionRow(action.title, action.description,
                ...
                onFocusChanged = focus(action.title, LauncherActionMeaning.OPEN_SETTINGS, { callbacks.onOpenSystemAction(action.key) }, callbacks.onFocusedAction)) }
        }
    }
}

private fun focus(label: String, meaning: LauncherActionMeaning, action: () -> Unit, callback: OnFocusedAction): (Boolean) -> Unit = { focused -> callback(if (focused) FocusedControlAction(LauncherActionDescriptor(SemanticInputAction.CONFIRM, meaning, label), action) else null) }
