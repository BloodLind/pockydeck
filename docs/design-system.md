...
The small-text correction applies a tapered increase after reference scaling: 3sp becomes 6sp, 9sp becomes 11sp, and the increase reaches zero at 14sp. Intermediate sizes interpolate between those anchors. The latest queued revision then increases all type roles by 8%, with proportional line heights and normal Android font scaling. Clock/status text remains regular weight. Footer hint groups and their icon-to-label gaps increase slightly, with a small bottom inset; shell and Home artwork bounds remain fixed.

Library places its counted compact filter pills on the same row as the title. A bounded strip shows up to four choices and cycles through every populated console. Fixed L2/R2 hints sit outside the scrolling strip and are decorative, not buttons. All filters opens a three-column chooser with larger targets. The strip, chooser and grid share the same selection/count rules; the active filter remains visible. Reference consoles lead when populated; remaining consoles follow alphabetically. Sorting uses the shared compact icon/label/chevron control and an explicit Title/Recent chooser. Inactive pills retain the reference's muted treatment.
...
Recent activity badges describe observations honestly: an app launched here may show Recent, with the full Recently active meaning in accessibility text. A ROM identifies the emulator last dispatched for that game. Badges expire after 30 minutes; optional Usage Access can add app observations, but does not extend a ROM's launch timestamp. These observations do not establish that a process or game is currently running. Wi-Fi is icon-only and crosses out only when its radio is disabled; battery, temperature, memory and storage glyphs vary with their readings.
...
L2/R2 cycle filters on collection/search pages and move through cards on Home. A press moves once; holding starts repetition after 360ms and accelerates from 115ms to 55ms intervals over 2.5 seconds. D-pad and stick holds use the same acceleration. Digital and analog reports of one trigger share a held state with separate press/release thresholds, so a slow squeeze does not double-step. Direction/source handoff preserves elapsed hold time; release, editor entry and loss of activity reset repeating input.
