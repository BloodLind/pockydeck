...
Planning baseline: 7 September 2026; current behavior reconciled through v0.9. Reference measurements remain distinct from native dimensions. Calibration and acceptance evidence are recorded in [implementation progress](implementation/progress.md).
...
Every card has an unfocused perimeter outline; controller-focused cards use amber and a short lift/press transition, disabled under reduced motion. Collection artwork is centered and capped at 176 reference units, while two-line titles retain the wider column allocation. App and ROM collections share that geometry. The existing five/four/three-column breakpoints remain, with five columns on the Flip 2 at default scale; horizontal and vertical gaps use the smaller `xs` spacing. Library, Apps and Favorites each persist an independent Grid/List choice. List mode uses one column of the shared horizontal result cards while retaining item identity, filters, sorting and actions. APP, EMU, ANDROID and console abbreviations sit on the artwork, including Home; redundant type subtitles are removed. Console colors supplement written abbreviations such as GBA, SNES and PSX. Loaded artwork changes crossfade over 140ms, with an immediate replacement under Reduce motion.
