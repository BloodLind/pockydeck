...
Collection headers align title, a centered filter group, Grid/List and Sort in a row at least 48dp high, with the item count below. Native text measurement includes the actual scaled typography, padding, glyphs and 48dp targets; the header chooses a second filter row when those controls no longer fit, including at 100%, 110% and 120% UI scale. Fixed All precedes a bounded console group and fixed All filters follows it. The finite strip shows three complete natural-width category pills, retaining the last three at its end. Fewer populated categories or genuinely narrow space can reduce that count; no phantom or partial cells are added. Fixed decorative L2/R2 hints sit outside the scrolling area. All filters opens a three-column chooser with larger targets. The strip, chooser and content share selection/count rules; explicit filter changes can scroll the selected category into view, while D-pad focus never reveals clipped/offscreen chips. Reference consoles lead when populated; remaining consoles follow alphabetically. Sort uses the shared icon/label/chevron control and a Title/Recent chooser with an end-aligned action-sized Close button. Inactive pills retain the reference's muted treatment.
