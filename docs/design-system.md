...
### 8 September v0.8 readability and interaction revision
...
Library aligns its title, compact filters and Sort in one row at least 48dp high, with the item count below. Compact widths retain a second filter row. All is a separate fixed shortcut, followed by a divider and a finite horizontal strip of the remaining populated categories. Pills use their natural text width; the strip reserves about 72 reference units less than the available space, roughly one short category, to separate the chooser and Sort. Fixed decorative L2/R2 hints sit outside the scrolling area. All filters opens a three-column chooser with larger targets. The strip, chooser and grid share the same selection/count rules; an explicit filter change can scroll the selected category into view, while D-pad focus never scrolls clipped or offscreen chips into view. Reference consoles lead when populated; remaining consoles follow alphabetically. Sorting uses the shared compact icon/label/chevron control and an explicit Title/Recent chooser. Inactive pills retain the reference's muted treatment.

Every card has an unfocused perimeter outline; controller-focused cards use amber and a short lift/press transition, disabled under reduced motion. Collection artwork is centered and capped at 208 reference units, while two-line titles retain the wider column allocation. App and ROM collections share that geometry. The existing five/four/three-column breakpoints remain, with five columns on the Flip 2 at default scale; horizontal and vertical gaps use the smaller `xs` spacing. APP, EMU, ANDROID and console abbreviations sit on the artwork, including Home; redundant type subtitles are removed. Console colors supplement written abbreviations such as GBA, SNES and PSX. Loaded artwork changes crossfade over 140ms, with an immediate replacement under Reduce motion.
...
| Settings | Category list and grouped setting rows | Persisted UI scale and Reduce motion, controller mapping, default-HOME setup, notification-indicator access, ROM sources, emulators and artwork |
...
The collection header's D-pad Left/Right path is fixed All → fully visible category chips → All filters → Sort, and reverses in the opposite direction. Clipped and offscreen chips are excluded; moving focus does not implicitly scroll the strip. Down restores the selected or first settled card, falling back to All if none survives. L2/R2 change the selected filter through the full finite sequence.
...
After active editor/modal handling, Back on dock-opened Search or Settings returns to Home. Back on Home, Library, Apps or Favorites preserves the current page, focus, selection and viewport. Shortcut Search and item details retain explicit origins and Back restores those origins.
