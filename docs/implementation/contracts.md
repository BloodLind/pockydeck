...
- Back on any of the six dock destinations is a no-op once active editor/modal handling is complete; it preserves that page's focus, selection and viewport.
...
Collection filter counts share the grid's availability, category and favorite rules. Fixed All precedes a finite, horizontally scrolling strip of natural-width category pills; All filters opens the complete three-column chooser. Decorative L2/R2 hints remain outside the strip. The active category scrolls into view, and trigger navigation uses the same filter-key order without wrapping at either end. The root owns the filter modal, including mapped Back/Confirm handling and opener focus restoration. Exact known frontend packages ES-DE, GameNative and Artemis default to Apps even if Android declares them as games; explicit user category overrides remain authoritative.
...
## 0.7 controller, layout and display preferences
...
L2/R2 step through collection/Search filters and move horizontally on Home. Filter selection clamps at either end without resetting the grid anchor. While the collection's scrolling filter strip has focus, Left/Up requests fixed All, Right requests All filters (or Sort when the chooser is absent), and Down restores the selected/first card, falling back to All if none survives. In the card grid, Right/Left follow reading order across rows and stop at the whole collection's ends. Up/Down preserve the column and allow exit at vertical edges. One conflated navigation worker follows the latest requested key, using bounded scrolling before focusing a placed item; it never queues an animation per repeat. Controller dock shortcuts change destination selection without requesting dock focus. Root Back is a no-op on all six dock pages; shortcut Search and details still restore their explicit origins after editor/modal handling.
...
Library's header contains its title, fixed All, a finite compact filter strip, All filters and the shared SortSelector. Category pills use natural widths; fixed decorative L2/R2 hints are spaced outside the scrolling region. Header height grows with text; below 560dp content width, filters occupy a second row so they retain space. All filters uses three columns of larger targets. Collection cards use centered square artwork capped at 176 reference units and wider two-line captions. Type/console tags sit on the artwork. Typography adds a further 10% after the prior 8% increase and tapered small-text adjustment, preserving regular clock/status weights and the official light icons. Dock controls/glyphs are 10% larger than the previous revision. Footer action content is raised by eight reference units inside a band of at least 52dp.

Filter selection explicitly requests resulting-card focus. Restoration follows the request, filter and sort, waits for completed results, validates the placed stable item key and retries for a bounded number of frames. A newer criterion, touch input or modal cancels that request. Dock outline suppression during bumper changes affects presentation only: native focus targets remain available so Android's early key handling does not swallow the first direction after touch. Back hints are shown only when an editor, modal or explicit return route can consume them.
