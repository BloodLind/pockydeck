...
Collection filter counts share the grid's availability, category and favorite rules. The primary strip has at most six counted pills with More for overflow; its active overflow selection remains visible. L2/R2 and touch arrows use the same filter-key order. The root owns the filter modal, including mapped Back/Confirm handling and opener focus restoration. Exact known frontend packages ES-DE, GameNative and Artemis default to Apps even if Android declares them as games; explicit user category overrides remain authoritative.
...
Search uses a local editor buffer and an edit-entry query snapshot while showing live matches. Apply commits the current query; Cancel restores its entry query. Both close the IME and, in controller mode, return focus to results or the compact Edit search action. The edit session survives field blur so footer Apply/Cancel still work. Downward result scrolling collapses the header; upward scrolling and explicit editing reveal it. Mapped gamepad A/B reach these actions during IME editing; keyboard text/caret keys remain native.

Collection indexing, filtering and sorting use the injected computation dispatcher (Default in production), separate from light selection/viewport state. Search publishes 128 initial matches, then up to 256 more per 24ms yield. A new query cancels the previous publication; results are not shown against mismatched query text. Prepared lists and membership sets are reused across navigation. Scroll observers save anchors without recomposing an entire screen for every pixel.

## 0.6 controller, layout and activity observations

Digital and analog reports are combined into per-device held controls. Triggers use 0.55 engagement and 0.25 release thresholds. A press emits once; held controls repeat after 360ms, accelerating from 115ms to 55ms intervals over 2.5 seconds. D-pad, hat and stick ownership is resolved consistently and same-direction source handoff retains the hold timer. Release, IME ownership and lifecycle reset cancel repetition; held axes suppressed by a reset must return to neutral before rearming.

L2/R2 cycle collection/Search filters and move horizontally on Home. Right/Left follow grid reading order across rows and stop at the whole collection's ends. Up/Down preserve the column and allow exit at vertical edges. One conflated navigation worker follows the latest requested key, using bounded scrolling before focusing a placed item; it never queues an animation per repeat. Controller dock shortcuts change destination selection without requesting dock focus.

MainActivity owns a native pre-IME input host so Android cannot consume the first post-touch directional key before the launcher sees it. The same handler still lets editor caret/text input pass through. Landscape text input requests no fullscreen/no extract UI, and the compact shell reserves a 48dp Apply/Cancel footer above the keyboard when at least 128dp remains for the combined query/results/footer layout. Touch taps and the footer use the same edit-session actions as mapped controller A/B.

Library's header contains its title, a bounded cyclic filter strip with fixed decorative L2/R2 hints, All filters and the shared SortSelector. All filters uses three columns of larger targets. Collection cards use centered square artwork capped at 176 reference units and wider two-line captions; Home keeps its original artwork allocation. Type/console tags sit on the artwork. Typography adds 8% after the existing tapered small-text adjustment, preserving regular clock/status weights and the official light icons.

`RecentActivityTracker` records only acknowledged Android dispatches and the most recent ROM dispatched to each emulator. Its local preferences are excluded from backup. Optional, user-granted Usage Access contributes app activity observations; it never establishes live process state, identifies a different ROM, or extends a ROM's original dispatch timestamp. Labels expire after 30 minutes; clock-skewed future records are ignored. ROM labels include the dispatched emulator name. Polling is lifecycle-bound, and permission revocation removes queried observations. No Running or guaranteed Resume claim is made.

Wi-Fi's crossed glyph follows radio enabled state rather than connection presence. Status glyphs use raw battery, temperature, memory and storage readings, retaining accessible names/values when the visible Wi-Fi label is omitted.
