...
The **0.9.0 (code 12)** revision preserves all available recent Home entries before a stable showcase with one game per otherwise unrepresented console, keeps the 20-card limit and resets Home to its first card on every return. Collection headers measure the actual scaled controls; smaller grid cards and independent Grid/List preferences are available in Library, Apps and Favorites. Page entries use a short fade/slide, Launcher category totals are informational, and Sort uses a compact Close action.
...
The **0.10.2 (code 16)** correction gives List previews grouped round Open, Details and Favorite controls, makes explicit text actions visibly fill their targets with larger labels, and shows three complete console categories with a full final window. Header, filter and footer styling otherwise follows the established design. Real-screen review also fixed stretched preview glyphs and reconciled centered text/accessibility bounds.
