...
Implement Search route/state/ViewModel query, category, and platform state for local catalog matching across All/Games/Apps/System. Deduplicate by stable ID, retain query on category changes, show platform filtering only when relevant, and render compact cards with useful empty-query recents, match, no-result, and restricted-filter states.
...
- [ ] **AC-04** — **Given** empty query, no matching results, or a restricted filter with no results, **when** Search renders, **then** it shows established recents or a state-specific usable recovery; internal system shortcuts never enter Home recency.
...
- [ ] **AC-06** — **Given** Search is used offline, **when** matching occurs, **then** it reads local indexed data without network requests or access inside another app and cards keep stable dimensions.
...
- **AC-04:** Not run — Compose tests for recents, no result, restricted filter, and recency exclusion.
